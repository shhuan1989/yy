...
class MeetingGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Meeting entity")
        ...
        .repeat(2) {
            ...
            .post("/api/meetings")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "startTime":null, "endTime":null, "info":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_meeting_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
