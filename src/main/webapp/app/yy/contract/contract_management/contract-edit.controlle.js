(function() {
    ...
    ContractEditController.$inject = ['$timeout', '$scope', '$rootScope', '$state', '$stateParams', '$q', 'disableEdit', 'ContractManagementService', 'FileService', 'DictionaryService'];

    function ContractEditController ($timeout, $scope, $rootScope, $state, $stateParams, $q, disableEdit, ContractManagementService, FileService, DictionaryService) {
        const vm = this;
        ...
        function initInstallmentTimePicker(index) {
            $scope.$$postDigest(function() {
                $('#field_installment_time_'+index).datetimepicker({
                    format: dateFormatDMY(),
                    locale: 'zh-CN',
                    defaultDate: datetimePickerDefaultTime(vm.contract.installments[index].eta)
                }).on('dp.change', function(ev){
                    vm.contract.installments[index].eta = moment(ev.date).valueOf();
                });
            });
        }
