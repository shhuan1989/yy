...
var gulp = require('gulp'),
    ...
    babel = require("gulp-babel"), // Use next generation JavaScript, today, with Babel, es6 to es5
...
gulp.task('clean', function () {
    return del([config.dist, config.babel], { dot: true });
});

gulp.task('babel', function () {
    return gulp.src(config.app + 'app/**/*.js')
        .pipe(babel())
        .pipe(gulp.dest(config.babel + 'app/'));
});
...
gulp.task('watch', function () {
    ...
    gulp.watch(config.app + 'app/**/*.js', ['babel', 'inject:app']);
    //
    ...
    // gulp.watch([config.app + '*.html', config.app + 'app/**', config.app + 'i18n/**', config.babel + 'app/**']).on('change', browserSync.reload);
});
...
gulp.task('build', ['clean'], function (cb) {
    runSequence(['copy', 'babel', 'inject:vendor', 'ngconstant:prod', 'copy:languages'], 'inject:app', 'inject:troubleshoot', 'assets:prod', cb);
});
