...
var injectPage = [config.app + 'index.html', config.app +'lockscreen.html', config.app +'login.html'];
...
function app() {
    return gulp.src(injectPage)
        .pipe(inject(gulp.src(config.babel + 'app/**/*.js')
            .pipe(naturalSort())
...
function vendor() {
    var stream = gulp.src(injectPage)
