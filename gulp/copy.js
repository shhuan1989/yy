...
function fonts() {
    return es.merge(
        gulp.src([
            ...
            config.app + 'content/**/*.{woff,woff2,svg,ttf,eot,otf}',
            config.app+'bower_components/font-awesome/fonts/*.{otf,eot,svg,ttf,woff,woff2}' // miss woff2 will leads OTS parsing error: invalid version tag
        ])
        .pipe(plumber({errorHandler: handleErrors}))
        .pipe(debug())
...
function images() {
    ...
    return es.merge(
        gulp.src(bowerFiles({filter: ['**/*.{gif,jpg,png}']}), { base: config.bower })
            .pipe(plumber({errorHandler: handleErrors}))
            .pipe(changed(config.dist +  'bower_components'))
            .pipe(gulp.dest(config.dist +  'bower_components')),
        gulp.src(config.app + 'content/**/*.{gif,jpg,png}')
            .pipe(plumber({errorHandler: handleErrors}))
